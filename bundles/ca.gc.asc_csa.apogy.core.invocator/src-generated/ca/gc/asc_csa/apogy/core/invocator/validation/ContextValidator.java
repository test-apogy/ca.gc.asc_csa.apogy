/**
 *
 * $Id$
 */
package ca.gc.asc_csa.apogy.core.invocator.validation;

import ca.gc.asc_csa.apogy.core.invocator.ContextsList;
import ca.gc.asc_csa.apogy.core.invocator.DataProductsList;
import ca.gc.asc_csa.apogy.core.invocator.Environment;
import ca.gc.asc_csa.apogy.core.invocator.VariableImplementationsList;

import java.util.Date;

/**
 * A sample validator interface for {@link ca.gc.asc_csa.apogy.core.invocator.Context}.
 * This doesn't really do anything, and it's not a real EMF artifact.
 * It was generated by the org.eclipse.emf.examples.generator.validator plug-in to illustrate how EMF's code generator can be extended.
 * This can be disabled with -vmargs -Dorg.eclipse.emf.examples.generator.validator=false.
 */
public interface ContextValidator {
	boolean validate();

	boolean validateContextsList(ContextsList value);
	boolean validateVariableImplementationsList(VariableImplementationsList value);
	boolean validateEnvironment(Environment value);
	boolean validateDataProductsList(DataProductsList value);
	boolean validateInstancesCreationDate(Date value);
	boolean validateInstancesDisposalDate(Date value);
}
