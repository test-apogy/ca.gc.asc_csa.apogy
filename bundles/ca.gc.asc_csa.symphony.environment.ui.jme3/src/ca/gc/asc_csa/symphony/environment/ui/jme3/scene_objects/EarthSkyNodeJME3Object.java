package ca.gc.asc_csa.symphony.environment.ui.jme3.scene_objects;
...
import ca.gc.asc_csa.symphony.environment.ui.jme3.preferences.SymphonyEnvironmentUIJME3PreferencesConstants;
...
{							
	...
	public EarthSkyNodeJME3Object(EarthSkyNode node, JME3RenderEngineDelegate jme3RenderEngineDelegate) 
	{
			...
			{														
				jme3Application.enqueue(new Callable<Object>() 
				{
					...
					{			
						...
						// Blooming
						enableBloom(ca.gc.asc_csa.symphony.environment.ui.jme3.Activator.getDefault().getPreferenceStore().getBoolean(SymphonyEnvironmentUIJME3PreferencesConstants.DEFAULT_BLOOM_ENABLED_ID));										
					   						
						// Shadow map.
						setShadowMapSize(ca.gc.asc_csa.symphony.environment.ui.jme3.Activator.getDefault().getPreferenceStore().getInt(SymphonyEnvironmentUIJME3PreferencesConstants.DEFAULT_SHADOW_MAP_SIZE_ID));
						
					    // Shadows from Sun				
						setSunShadowsEnabled(ca.gc.asc_csa.symphony.environment.ui.jme3.Activator.getDefault().getPreferenceStore().getBoolean(SymphonyEnvironmentUIJME3PreferencesConstants.DEFAULT_SUN_CAST_SHADOWS_ENABLED_ID));
		
						// Shadows from Moon.
						setMoonShadowsEnabled(ca.gc.asc_csa.symphony.environment.ui.jme3.Activator.getDefault().getPreferenceStore().getBoolean(SymphonyEnvironmentUIJME3PreferencesConstants.DEFAULT_MOON_CAST_SHADOWS_ENABLED_ID));
		...
		ca.gc.asc_csa.symphony.environment.ui.jme3.Activator.getDefault().getPreferenceStore().addPropertyChangeListener(this);		
	}
	...
	{				
		jme3Application.enqueue(new Callable<Object>() 
		{
			...
			public Object call() throws Exception 
			{
				...
				{
					boolean value = ca.gc.asc_csa.symphony.environment.ui.jme3.Activator.getDefault().getPreferenceStore().getBoolean(SymphonyEnvironmentUIJME3PreferencesConstants.DEFAULT_BLOOM_ENABLED_ID);
					enableBloom(value);
					updateSun();
				}
				...
				{
					boolean value = ca.gc.asc_csa.symphony.environment.ui.jme3.Activator.getDefault().getPreferenceStore().getBoolean(SymphonyEnvironmentUIJME3PreferencesConstants.DEFAULT_SUN_CAST_SHADOWS_ENABLED_ID);
				...
				{
					boolean value = ca.gc.asc_csa.symphony.environment.ui.jme3.Activator.getDefault().getPreferenceStore().getBoolean(SymphonyEnvironmentUIJME3PreferencesConstants.DEFAULT_MOON_CAST_SHADOWS_ENABLED_ID);
	...
	private Geometry getHorizon()
	{
		if(horizon == null)
		{
			Mesh horizonMesh = JME3PrimitivesUtilities.createSphericalCap(HORIZON_RADIUS, (float) Math.toRadians(-90), 0f, 32);
	...
	private Geometry getAtmosphere()
	{
		if(atmosphere == null)
		{
			Sphere sphere = new Sphere(10, 36, ATMOSPHERE_RADIUS);			
	...
	{				
		if(sunSphere == null)
		{
			float sunRadius = (float) (SUN_AND_MOON_RADIUS * Math.tan(earthSky.getSunAngularDiameter() / 2.0) );
	...
	{				
		if(moonSphere == null)
		{
			float moonRadius = (float) (SUN_AND_MOON_RADIUS * Math.tan(earthSky.getMoonAngularDiameter() / 2.0) );
