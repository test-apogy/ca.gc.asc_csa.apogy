 ...
 * Surface Environment Facade</b></em>'. <!-- end-user-doc -->
 *
 * @generated
 */
public class ApogySurfaceEnvironmentUIFacadeImpl extends MinimalEObjectImpl.Container
		implements ApogySurfaceEnvironmentUIFacade {
	/**
	 * <!-- begin-user-doc --> <!-- end-user-doc -->
	 * @generated
	 */
	protected ApogySurfaceEnvironmentUIFacadeImpl() {
		super();
	}
	 ...
	 * @generated_NOT
	 */
	public List<RectangularRegionProvider> getVisibleRectangularRegionProvider(
	 ...
	 * @generated_NOT
	 */
	public AbstractEImage getImageMapLayerPresentationImage(MapViewConfiguration mapViewConfiguration,
			RectangularRegion mapViewExtent, int maximumImageSizePixels) {
		AbstractEImage result = null;

		// Gets the image covered by the specified list of
		...
		double horizontalResolution = (mapImage.getWidth()
				/ mapViewConfiguration.getMapImageRectangularRegion().getXDimension());
		double verticalResolution = (mapImage.getHeight()
				/ mapViewConfiguration.getMapImageRectangularRegion().getYDimension());

		// If the specified mapViewExtent is entirely covered by the
		// MapViewConfiguration.
		if (ApogySurfaceEnvironmentFacade.INSTANCE.fitsInside(mapViewExtent,
				mapViewConfiguration.getMapImageRectangularRegion())) {
			// Gets the sub image representing mapViewExtent within
			// mapViewConfiguration.
			...
			int x = (int) Math.round(
					(minX - mapViewConfiguration.getMapImageRectangularRegion().getXMin()) * horizontalResolution);
			int y = mapImage.getHeight() - (int) Math
					.round((maxY - mapViewConfiguration.getMapImageRectangularRegion().getYMin()) * verticalResolution);
			if (x < 0)
				x = 0;
			if (y < 0)
				y = 0;
			...
			try {
				...
				if (width * height > maximumImageSizePixels) {
					// Rescale the image.
					double scaleFactor = Math
							.sqrt((double) maximumImageSizePixels / (mapImage.getWidth() * mapImage.getHeight()));
					result = EImagesUtilities.INSTANCE.resize(result, scaleFactor);
				}
			} catch (Throwable t) {
				Logger.INSTANCE.log(Activator.ID, this, "getImageMapLayerPresentationImage failed!",
						EventSeverity.ERROR, t);
				t.printStackTrace();
			}
		} else {
			// Finds the size of the image that covers the specified
			// MapViewExtent.
			...
			result = EImagesUtilities.INSTANCE.createUniformColorImage(resultWidth, resultHeight, red, green, blue,
					255);

			// Computes how much the mapImage needs to be translated.
			double xTranslation = mapViewConfiguration.getMapImageRectangularRegion().getXMin()
					- mapViewExtent.getXMin();
			int widthTranslation = (int) Math.round((xTranslation) * horizontalResolution * scaleFactor);

			double yTranslation = mapViewExtent.getYMax()
					- mapViewConfiguration.getMapImageRectangularRegion().getYMax();
	...
	public Object eInvoke(int operationID, EList<?> arguments) throws InvocationTargetException {
		switch (operationID) {
			case ApogySurfaceEnvironmentUIPackage.APOGY_SURFACE_ENVIRONMENT_UI_FACADE___GET_VISIBLE_RECTANGULAR_REGION_PROVIDER__MAPVIEWCONFIGURATION:
				return getVisibleRectangularRegionProvider((MapViewConfiguration)arguments.get(0));
			case ApogySurfaceEnvironmentUIPackage.APOGY_SURFACE_ENVIRONMENT_UI_FACADE___GET_IMAGE_MAP_LAYER_PRESENTATION_IMAGE__MAPVIEWCONFIGURATION_RECTANGULARREGION_INT:
			...
			case ApogySurfaceEnvironmentUIPackage.APOGY_SURFACE_ENVIRONMENT_UI_FACADE___GET_TRAJECTORY_LENGTH__XYSERIES:
				return getTrajectoryLength((XYSeries)arguments.get(0));
			case ApogySurfaceEnvironmentUIPackage.APOGY_SURFACE_ENVIRONMENT_UI_FACADE___GET_MAP_VIEW_CONFIGURATION_IDENTIFIER__MAPVIEWCONFIGURATION:
				return getMapViewConfigurationIdentifier((MapViewConfiguration)arguments.get(0));
			case ApogySurfaceEnvironmentUIPackage.APOGY_SURFACE_ENVIRONMENT_UI_FACADE___GET_ACTIVE_MAP_VIEW_CONFIGURATION__STRING:
				return getActiveMapViewConfiguration((String)arguments.get(0));
			case ApogySurfaceEnvironmentUIPackage.APOGY_SURFACE_ENVIRONMENT_UI_FACADE___GET_ACTIVE_MAP_VIEW_CONFIGURATION_LIST:
				return getActiveMapViewConfigurationList();
			case ApogySurfaceEnvironmentUIPackage.APOGY_SURFACE_ENVIRONMENT_UI_FACADE___GET_FEATURE_OF_INTEREST_LISTS__INVOCATORSESSION:
