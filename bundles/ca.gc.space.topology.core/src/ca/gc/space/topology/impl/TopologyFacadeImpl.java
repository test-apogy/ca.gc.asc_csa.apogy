 ...
 * </copyright>
 *
 * $Id: TopologyFacadeImpl.java,v 1.10.2.3 2015/02/03 20:01:30 rlarcheveque Exp $
 */
 ...
 * @generated
 */
public class TopologyFacadeImpl extends EObjectImpl implements TopologyFacade {

	private static TopologyFacade instance;

	public static TopologyFacade getInstance() {
		if (instance == null) {
			instance = new TopologyFacadeImpl();
		}
	...
	public RotationNode createRotationNodeXYZ(double x, double y, double z) {
		...
		Matrix3x3 matrix = MathFacade.INSTANCE.createMatrix3x3(rotationMatrix);
	 ...
	 * @generated_NOT
	 */
	public double getEuclidianDistance(Node fromNode, Node toNode) 
	{
		if(fromNode == toNode)
		{
			return 0.0;
	...
	public double getGeodesicDistance(Node fromNode, Node toNode) 
	{
		...
		else
		{
			...
			for(int i = 0; i < nodePath.size(); i++ )
			{
				...
				{
					distance += getEuclidianDistance(previousNode, currentNode);
				}			
	...
	public Object eInvoke(int operationID, EList<?> arguments)
		...
		switch (operationID) {
			...
			case TopologyPackage.TOPOLOGY_FACADE___GET_EUCLIDIAN_DISTANCE__NODE_NODE:
				return getEuclidianDistance((Node)arguments.get(0), (Node)arguments.get(1));
	...
	private Matrix4d computeMatrix(List<Matrix4d> matrices, boolean reverse) {
		...
		result.setIdentity();
		// System.out.println("TopologyFacadeImpl.computeMatrix()");
		...
		while (!matrices.isEmpty()) {
			...
			// System.out.println("TopologyFacadeImpl.computeMatrix() multiplying "
			// + matrix);
			result.mul(matrix);
		}
