...
public class QuickHull3D {
	 ...
	 */
	public void build(Vector3d[] points, int nump)
			throws IllegalArgumentException {
		if (nump < 4) {
			throw new IllegalArgumentException(
					"Less than four input points specified");
	...
	protected void createInitialSimplex() throws IllegalArgumentException {
		...
		for (int i = 0; i < 3; i++) {
			...
			maxVtxs[i].pnt.get(maxPiData);
			minVtxs[i].pnt.get(minPiData);
	...
	protected void calculateHorizon(Vector3d eyePnt, HalfEdge edge0, Face face,
			List<HalfEdge> horizon) {
		// oldFaces.add (face);
