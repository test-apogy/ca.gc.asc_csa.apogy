 ...
 * software.
 */
package ca.gc.space.math.quickhull3d;
...
public class QuickHull3DTest {
	...
	private void setRandom(Vector3d v, Random rand, double min, double max) {

		double range = max - min;

		v.x = rand.nextDouble() * range + min;
		v.y = rand.nextDouble() * range + min;
		v.z = rand.nextDouble() * range + min;
	}
	...
	public double[] randomSphericalPoints(int num, double radius) {
		...
		for (int i = 0; i < num;) {
			setRandom(pnt, rand, -radius, radius);
		...
		for (int i = 0; i < gridSize; i++) {
			for (int j = 0; j < gridSize; j++) {
				for (int k = 0; k < gridSize; k++) {
					coords[idx * 3 + 0] = (i / (double) (gridSize - 1) - 0.5)
							* width;
					coords[idx * 3 + 1] = (j / (double) (gridSize - 1) - 0.5)
							* width;
					coords[idx * 3 + 2] = (k / (double) (gridSize - 1) - 0.5)
							* width;
					idx++;
				}
			}
	...
	void explicitFaceCheck(QuickHull3D hull, int[][] checkFaces)
		...
		if (faceIndices.length != checkFaces.length) {
			throw new Exception("Error: " + faceIndices.length + " faces vs. "
					+ checkFaces.length);
		}
	...
	void rotateCoords(double[] res, double[] xyz, double roll, double pitch,
			double yaw) {
	...
	void printCoords(double[] coords) {
		...
		for (int i = 0; i < nump; i++) {
			System.out.println(coords[i * 3 + 0] + ", " + coords[i * 3 + 1]
					+ ", " + coords[i * 3 + 2] + ", ");
		}
	}
	...
	void test(double[] coords, int[][] checkFaces) throws Exception {
		double[][] rpyList = new double[][] { { 0, 0, 0 }, { 10, 20, 30 },
				{ -45, 60, 91 }, { 125, 67, 81 } };
		...
		if (testRotation) {
			for (int i = 0; i < rpyList.length; i++) {
				...
				rotateCoords(xcoords, coords, Math.toRadians(rpy[0]), Math
						.toRadians(rpy[1]), Math.toRadians(rpy[2]));
	...
	public void explicitAndRandomTests() {
		try {
			...
			coords = new double[] { 21, 0, 0, 0, 21, 0, 0, 0, 0, 18, 2, 6, 1,
					18, 5, 2, 1, 3, 14, 3, 10, 4, 14, 14, 3, 4, 10, 10, 6, 12,
					5, 10, 15, };
			test(coords, null);

			coords = new double[] { 0.0, 0.0, 0.0, 21.0, 0.0, 0.0, 0.0, 21.0,
					0.0, 2.0, 1.0, 2.0, 17.0, 2.0, 3.0, 1.0, 19.0, 6.0, 4.0,
					3.0, 5.0, 13.0, 4.0, 5.0, 3.0, 15.0, 8.0, 6.0, 5.0, 6.0,
					9.0, 6.0, 11.0, };
			test(coords, null);
